...
txtUserName.addEventListener('keyup', () => {
    txtUserName.value && txtUserName.value.length >= 3
...
btnSendName.addEventListener('click', async () => {
    ...
    textToSpeach(`Hola ${userName}. ¿Qué necesitas?`);
    connectarSocket();
});
...
const connectarSocket = () => {
    ...
    socket.on('receive-transcript', (message) => {
        chatContent.innerHTML += answerUser(message);
    });

    socket.on('receive-completion', (completion) => {
